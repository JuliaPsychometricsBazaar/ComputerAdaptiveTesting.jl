...
function make_resp(raw)
    [Response(findfirst((hlabel) -> hlabel == label, empirical_irf.labels),
        resp)
...
function sample_words(empirical_irf; num_words = 12, limit = nothing)
    ...
    rand([idx for (idx, word) in enumerate(label) if match(WORD_RE, word) !== nothing],
        num_words)
end
...
function plot_bootstrapped(empirical_irf, idxs)
    ...
    xz = data(df) * mapping(:freqs, layout = :words) *
         AlgebraOfGraphics.density(bandwidth = 0.04)
    axis = (; ylabel = "")
    draw(xz; axis)
